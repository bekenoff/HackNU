!(function (t, e, i, n) {
    ...
    function toggleTranslateDisplay(element) {
        var displayy = element.nextElementSibling; // Get the associated number display
        if (displayy.style.display === 'none') {
            // Hide all other number displays
            document.querySelectorAll('.number-display').forEach(function(div) {
                div.style.display = 'none';
            });
            displayy.style.display = 'block'; // Show this one
            console.log(1);
        } else {
            displayy.style.display = 'none'; // Hide this one
        }
    }

    function translateText() {
        var translatedText = document.getElementById('translatedText');
        var translationNote = document.getElementById('translationNote');
    
        // Check the current visibility and toggle accordingly
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const form = document.querySelector('form[action="/count_correct_answers"]');
        const formGrammar = document.querySelector('form[action="/count_correct_answers_grammar"]');
        ...
        form.addEventListener("submit", function(event) {
          ...
          for (const [key, value] of Object.entries(correctAnswers)) {
            const userAnswer = form.querySelector(`input[name="${key}"]`).value.trim();
          ...
          if(score == Object.keys(correctAnswers).length){
            alert(`Вы получаете +1 очко за прохождение теста`);
          }
          else{
            alert(`Вы получили ${score} из ${Object.keys(correctAnswers).length}. Попробуйте ещё раз`);
          }
        });


        formGrammar.addEventListener("submit", function(event) {
            event.preventDefault(); // Prevent the form from submitting normally
      ...
      document.addEventListener("DOMContentLoaded", function() {
        ...
        formGrammar.addEventListener("submit", function(event) {
            ...
            // Iterate over the correct answers and compare with user input
            for (const [key, value] of Object.entries(correctAnswers)) {
              const userAnswer = formGrammar.querySelector(`input[name="${key}"]`).value.trim();
              if (userAnswer.toLowerCase() === value.toLowerCase()) {
                score++; // Increment the score if the answer is correct
              }
            }
      ...
      function hideForm() {
        var form = document.querySelector('form[action="/count_correct_answers"]');
        ...
        function hideFormGrammar() {
            var form = document.querySelector('form[action="/count_correct_answers_grammar"]');
...
window.onload = function() {
    ...
    recognition.onresult = function (event) {
        ...
        transcriptElement.textContent = ''; // Clear the previous text
        for (let i = event.resultIndex; i < event.results.length; i++) {
            const transcript = event.results[i][0].transcript;
            transcriptElement.textContent += transcript; // Display recognized speech
        }
    };
        ...
        button.addEventListener('click', () => {
            if (isListening) {
                ...
                document.getElementById('start-btn').addEventListener('click', function () {
                    ...
                    fetch('http://localhost:4000/get-audio-response', {method: 'GET'})
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        loginForm.addEventListener('submit', function (event) {
            ...
            // Preparing the data to send in the format the server expects
            const postData = {
                client_mail: email,
            ...
            fetch('http://localhost:4000/api/auth/client', {
                method: 'POST',
                headers: {
    ...
    function fetchLevelData() {
        let id = 19;
        const url = `http://localhost:4000/api/progress/get?id=${id}`;


        fetch(url)
            .then(response => response.json())
            .then(data => {
                const element = document.getElementById('levelData');
                const summary = data.level;
                element.textContent = summary;
            })
            .catch(error => {
                console.error('Error fetching data:', error);
                document.getElementById('levelData').textContent = 'Failed to load data';
            });

        fetch(url)
            .then(response => response.json())
            .then(data => {
                const element = document.getElementById('pointsData');
                const summary = data.points;
                element.textContent = summary;
            })
            .catch(error => {
                console.error('Error fetching data:', error);
                document.getElementById('pointsData').textContent = 'Failed to load data';
            });

        fetch(url)
            .then(response => response.json())
            .then(data => {
                const element = document.getElementById('testsData');
                const summary = data.tests;
                element.textContent = summary;
            })
            .catch(error => {
                console.error('Error fetching data:', error);
                document.getElementById('testsData').textContent = 'Failed to load data';
            });

        fetch(url)
            .then(response => response.json())
            .then(data => {
                const element = document.getElementById('booksData');
                const summary = 0;
                element.textContent = summary;
            })
            .catch(error => {
                console.error('Error fetching data:', error);
                document.getElementById('booksData').textContent = 'Failed to load data';
            });

        fetch(url)
            .then(response => response.json())
            .then(data => {
                const element = document.getElementById('filmsData');
                const summary = data.films;
                element.textContent = summary;
            })
            .catch(error => {
                console.error('Error fetching data:', error);
                document.getElementById('filmsData').textContent = 'Failed to load data';
            });

        fetch(url)
            .then(response => response.json())
            .then(data => {
                const element = document.getElementById('meetingsData');
                const summary = data.meetings;
                element.textContent = summary;
            })
            .catch(error => {
                console.error('Error fetching data:', error);
                document.getElementById('meetingsData').textContent = 'Failed to load data';
            });

    }

    document.addEventListener('DOMContentLoaded', fetchLevelData);

    function sendText() {
        const inputElement = document.getElementById('inputText');
        const text = encodeURIComponent(inputElement.value);
        const url = `http://localhost:4000/api/correct-text?text=${text}`;

        fetch(url)
            .then(response => response.text()) // Processing as plain text instead of JSON
            .then(data => {
                console.log(data)
                // Update the content of the 'correctedText' element with the received data
                document.getElementById('correctedText').textContent = data;
            })
            .catch(error => {
                console.error('Error correcting text:', error);
                document.getElementById('correctedText').textContent = 'Error processing your text';
            });
    }

    function sendSyn() {
        const inputElement = document.getElementById('inputTextSyn');
        const text = encodeURIComponent(inputElement.value);
        const url = `http://localhost:4000/api/synonym?text=${text}`;
        ...
        fetch(url)
            ...
            .then(data => {
                console.log(data)
                // Update the content of the 'correctedText' element with the received data
                document.getElementById('synText').textContent = data;
            })
            .catch(error => {
                console.error('Error correcting text:', error);
                document.getElementById('synText').textContent = 'Error processing your text';
            });
    }
}
